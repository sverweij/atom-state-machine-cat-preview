...
export default class SmCatPreviewView extends ScrollView {
  ...
  grammar2inputType (pGrammar) {
    ...
    if (pGrammar) {
      lRetval = MAP[pGrammar.scopeName] || 'smcat'
    }
    return lRetval
  }
  ...
  renderSMCatText (text) {
    ...
    return renderer.render(text, { inputType: this.grammar2inputType(this.getGrammar()) }, (error, svg) => {
      if (error) {
  ...
  async saveAs (pOutputType) {
    ...
    const dialogOutput = await atom.applicationDelegate.showSaveDialog(
